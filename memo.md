...
### ファイル操作のルールとは
読み書きの操作があり, あとはファイルがない場合や権限の有無の内容で処理が失敗したりするという設定が施されている
`FILE型のポインタ変数 = fopen(ファイル名, モード);`
`fclose(FILE型のポインタ変数);`

### なぜfcloseするのか
pcが同時にアプリを開き同一ファイルを操作するときにファイル操作にロックをかけることができる。

### fileへの変数の内容の書き込み方法は
fprintf関数を使う
`fprintf(FILE型のポインタ変数, %dとか, 変数)`

### バイナリファイルの読み書き
### テキストとバイナリとは
すべてのファイルはバイナリであると言える。バイナリとは2進数ファイルのこと。数値のみで記録されたファイル。
テキストファイルは文字列だけが記録されたファイル。文字列だけでも文字コード
### ファイルとテキストとバイナリの関係性は
ファイルの中身はテキストかバイナリかの違いがある。テキストも文字列ということは変換されれば本質はバイナリ。
変換コストを考えれば初めからバイナリファイルであれば高速に動作することになる
### バイナリファイルとテキストファイルの操作において何か違いがあるのか
バイナリファイルをひらくときは最後に"b"をつける
### ファイルへの数値の書き込みについて
バイナリ解析用のエディタのプラグインをセットして読みこむ
61 => a
### 普通に100とかの数値を書き込むときと何か違うのか。

### ドラッグされたファイル名の取得とは
ターミナルにファイルをドラッグするとファイル名が取れる

### コマンドライン引数とは
アプリケーションを実行時に渡される文字列のこと。処理するファイル名や動作オプションを渡す

テキストファイル
バイナリファイル
file.datというファイルに 51 60, 52 60, 53, 60が書き込まれる

### 普遍の値の取り扱いとは

定数のこと
### 数値に名前をつけるとは
`#define 名前 値`でファイルの先頭に宣言する。閉じタグはつけない

### const定数とは

### defineとconst定数は何が違うのか
関数内の定数かglobalな定数かで違う

### enum定数とは
定数を複数宣言できる。そして順番も管理できる。フラグ定数として使われていて
1だったら男
2だったら女性
みたいな感じ

### define擬似命令の高度な使い方とは
`#define`擬似命令は定数として数値を定義するだけでなくなんでも置き換えが可能, globalな関数が実装できてしまう。

### マクロという簡易関数とは
`#define`で定義した関数の引数に値を渡すこと。

### そもそもマクロとは
`#define`擬似命令に夜置き換えで式などを簡単に表現すること
決まった数式などを定義するときに使えるがあまりやらない
...
要素数をプログラムの中で変更できない
### 何が問題か
メモリを確保しすぎたり, 逆に確保できなかったりすることが問題

### メモリを自由に確保するにはどうしたらいいか
malloc関数を使う
### malloc関数とは
...
### 動的配列とは
malloc関数を使って作成された任意のサイズの配列
### 確保されたメモリはいつまで残り続けるのか
プログラムが終了するまで残る。メモリの解放を忘れるとメモリが残り続けるので
free関数で解放する
`free(ポインタ変数)`
### なぜポインタをintにキャストしているのか
C++だったらエラーになるらしいのでキャストしている。ポインタと型をセットで使えと前回言っていたのでそれもあるかも。
### malloc関数はどのようにできているか
使えるメモリにマークをつけて確保しているだけ

### 動的配列の要素数を拡大するには
realloc関数を使う
`新しいポインタ変数 = realloc(以前のポインタ変数, 必要なメモリのバイト数)`
### reallocの問題点は
reallocを何回も呼び出すとメモリがフラグメンテーションという状態になり散らかる。
mallocで大きめにメモリを確保しておきreallocも一度だけ呼ぶようにする
...
### ヘッダーファイルとは
includeして使っていたstdio.hなどのヘッダーファイル。printfが使えるのはこのファイルをincludeしたため。
stdio.hには宣言文だけ書かれており, 実装は別のファイルにある。
関数や変数のみが宣言されたファイル。
...
ヘッダーファイルと, 実装ファイルを分けて作成する。実装ファイルをコンパイルしようとしてもmain関数がないのでできない
### コンパイルエラーになるけどどうすればいいか
コンパイルするときの引数に全てのソースコードファイルを含める必要がある。
`cc manager.c sum.c`のようにする。その時にヘッダーファイルが両方のファイルを繋いでくれる。インターフェースになっているということがすごくわかりやすい。
...
コンパイラが名前と形を記憶すること
## 定義とは
名前と形を記憶すると同時に変数や関数を作成すること
## プロトタイプ宣言を考えると
上記の宣言に該当するので定義は行われないよって幾つでもかける
## extern宣言とは
宣言だけして定義しない宣言。変数を別ファイルで宣言するときに使う
## 毎回変数を共有していいのか
せっかく機能ごとにファイルを分割する意味合いも薄れどこで変数を変更したかわからなくなるので控える

## ヘッダーファイルの重複インクルード自体を防ぐ方法とは
`#indef~#endif`を使う
ヘッダーファイル
ソースファイル
...
## 全体まとめ
おそらく読めるようにはなったと思う。
問題点としてやはりポインタ, ヘッダーファイルへの分割, 構造体の参照, extern, ifndefなどの利用具合がわかっていないので調べる。
